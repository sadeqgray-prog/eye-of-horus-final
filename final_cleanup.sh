...
echo -e "${BLUE}${BOLD}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  🧹 FINAL CLEANUP - REMOVE TOKEN FROM EVERYWHERE           ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${END}"

# ==================== مرحله ۱: جستجوی همه فایل‌های حاوی توکن ====================
...
find . -name "*.py" -exec sed -i 's/YOUR_TOKEN_HERE[a-zA-Z0-9]*/YOUR_TOKEN_HERE/g' {} \;
echo -e "${GREEN}✅ فایل‌های پایتون پاکسازی شدند${END}"

# ==================== مرحله ۴: پاکسازی فایل‌های txt ====================
echo -e "\n${YELLOW}[4/6] پاکسازی فایل‌های txt از توکن...${END}"

find . -name "*.txt" -exec sed -i 's/YOUR_TOKEN_HERE[a-zA-Z0-9]*/YOUR_TOKEN_HERE/g' {} \;
...
git add .
git commit -m "final: complete cleanup of all tokens"
